...
const EXACT_ZH_TO_EN: Record<string, string> = {
  ...
  "批量导入": "Batch import",
  "批量导入订阅": "Batch import subscriptions",
  "每行一个订阅链接": "One subscription URL per line",
  "名称正则": "Name regex",
  "用捕获组从链接提取名称，留空则使用域名中间段": "Use a capture group to name each URL. Leave blank to use the middle domain label.",
  "探测间隔": "Probe interval",
  "留空使用全局探测间隔，例如 30s": "Leave blank to use the global probe interval, for example 30s",
  "健康节点订阅": "Healthy node subscription",
  "手机客户端可导入 /<代理令牌>/api/v1/healthy-subscription 。默认是 base64 URI，format=sing-box 返回 sing-box JSON。":
    "Phone clients can import /<proxy token>/api/v1/healthy-subscription. The default is a base64 URI list; format=sing-box returns sing-box JSON.",
  "打开熔断": "Open circuit",
  "立即把这个节点标为熔断，不再分配。": "Mark this node circuit-open immediately so it is no longer assigned.",
  "确认把节点标为熔断？": "Mark this node as circuit-open?",
  "节点已熔断": "Node circuit opened",
...
Note: Once enabled, requests without authentication information are rejected instead of being treated as Default platform requests.`,
  ...
  "留空时使用默认 User-Agent：clash.meta。部分订阅源会校验客户端标识。":
    "Leave blank to use the default User-Agent: clash.meta. Some subscription providers check the client identity.",
  "User-Agent 不能超过 256 个字符": "User-Agent cannot exceed 256 characters",
  "User-Agent 包含非法字符": "User-Agent contains invalid characters",
